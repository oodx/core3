...
pub trait Read {
    ...
    #[cfg(all(feature = "alloc", feature = "utils"))]
    ...
    /// # Deprecation
    ///
    /// This method is carried over from `core2` for migration compatibility.
    /// It mirrors an unstable std API that was never stabilized. New
    /// implementations should not override this method.
    #[deprecated(since = "0.1.0", note = "legacy core2 compat, zero-initialize buffers before calling read() instead")]
    #[inline]
    #[allow(deprecated)]
...
/// # Deprecation
///
/// This type was carried over from `core2` for migration compatibility.
/// It mirrors an unstable std API (`#![feature(read_initializer)]`) that was
/// never stabilized and has since been removed. New code should not use it.
#[deprecated(since = "0.1.0", note = "legacy core2 compat, zero-initialize buffers before calling read() instead")]
#[derive(Debug)]
...
pub trait BufRead: Read {
    ...
    /// Read all bytes until a newline (the `0x0A` byte) is reached, and
    /// append them to the provided buffer.
    ///
    /// If successful, returns the total number of bytes read (including the
    /// trailing newline, if any).
    ///
    /// # Errors
    ///
    /// This function has the same error semantics as [`read_until`] and
    /// will also return an error if the read bytes are not valid UTF-8.
    ///
